...
public class HomeController {
	...
	public String inputData(@RequestParam("name") String name, @RequestParam("address") String address,
			@RequestParam("birthday_month") String birthdayMonth, @RequestParam("birthday_day") String birthdayDay,
	...
	public String modify(Locale locale, Model model, @RequestParam("idx") int idx) {
		...
		if (details != null) {
			...
			model.addAttribute("color_value", details.favorite_color);
	...
	public String delete(Locale locale, Model model, @RequestParam("idx") int idx) {
		DBCommon<Student> dbCommon = new DBCommon<Student>("c:\\tomcat\\student190527.sqlite", "student190527");
		dbCommon.deleteData(idx);
		return "done";
	}

	@RequestMapping(value = "/update_data", method = RequestMethod.GET)
	public String update_data(@RequestParam("idx") int idx, @RequestParam("name") String name,
			@RequestParam("address") String address, @RequestParam("birthday_month") String birthdayMonth,
			@RequestParam("birthday_day") String birthdayDay, @RequestParam("favoriteColor") String favoriteColor) {
		DBCommon<Student> dbCommon = new DBCommon<Student>("c:\\tomcat\\student190527.sqlite", "student190527");
